<?php
    ...
    function updateHistory($transactionDetails = [])
    {
        $details = $transactionDetails[0];
        if (!empty($details))
        {
            $insert = CS50::query("INSERT INTO history (user_id, transaction, symbol, shares, share_price) VALUES(?, ?, ?, ?, ?)", $details["userId"], $details["transaction"], strtoupper($details["symbol"]), $details["shares"], $details["share_price"]);
            if($insert != 0)
            {
                return true;
            }
