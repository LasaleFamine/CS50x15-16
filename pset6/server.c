...
char* indexes(const char* path)
{
    ...
    char* s1 = malloc(sizeof(char) * (lengthPath + 20));
    strcpy(s1, path);
    strcat(s1, "index.php");
    if (access(s1, F_OK) != -1) return s1;
    
    free(s1);
    s1 = malloc(sizeof(char) * (lengthPath + 20));
    strcpy(s1, path);
    strcat(s1, "index.html");
    if (access(s1, F_OK) != -1) return s1;
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    // allocate space for data loading
    ...
    for (int c = fgetc(file); c != EOF; c = fgetc(file))
    {
     ...
     // stores the address of the first of those bytes
    *content = data;
...
const char* lookup(const char* path)
{
    ...
    // lower case in case we have for ex .hTml
    for (int i = 0; i < strlen(path); i++)
    {
        newPath[i] = tolower(path[i]);
    }
    
    // check the mimes
    for(int i = 0; i < numberMimes; i++)
    {
        if (strstr(newPath, exts[i]) != NULL){
            index = i; 
        }
    }
    
    // Check and return the correct mimes
    if(index != -1)
    {
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char method[10], reqPath[100], httpReq[10];
    char absolute_path[LimitRequestLine + 1], queryEx[LimitRequestLine + 1];
    
    int spaces = 0;
    
    
    // Check for spaces in line
    // Must be only 2 (plus \t\r, so 4)
    ...
    int sscanReq = sscanf(line, "%s %s %s", method, reqPath, httpReq);
    if(sscanReq != 3)
    {
        error(400);
        return false;
    ...
    // Check if not '"' are present inside reqPath (so both path and queryEx)
    if(strchr(reqPath, '"') != NULL)
    {
        error(400);
        return false;
    }
    
    // Check for "/" in reqPath first position
    if(reqPath[0] != '/')
    {
        error(501);
        return false;
    ...
    // Extrapolate absolute_path and queryEx if there is
    int flag = 0;
    for(int i = 0; i < strlen(reqPath); i++)
    {
        // absolute_path or queryEx requires terminator because we take a single string with sscanf contains path and queryEx
        // so the terminator put from sscanf is only at the end of the "queryEx"
        if(i == strlen(reqPath) - 1)
        {
            if(flag == 0)
            {
                absolute_path[i + 1] = '\0';
        ...
        // Fill absolute_path or queryEx if there is
        if(reqPath[i] != '?' && flag == 0)
        {
            absolute_path[i] = reqPath[i];
        }
        else if(flag == 0) 
        {
            absolute_path[i] = '\0';
